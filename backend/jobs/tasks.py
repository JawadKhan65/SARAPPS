...
def send_heartbeat(redis_conn, crawler_id: str, job_id: str):
    ...
    try:
        ...
        redis_conn.hset(f"crawler:{crawler_id}:heartbeat", "job_id", job_id)
...
def update_job_progress(redis_conn, crawler_id: str, progress_data: Dict):
    ...
    try:
        ...
        pipeline.hset(
            f"crawler:{crawler_id}:progress", "current", progress_data.get("current", 0)
        )
        pipeline.hset(
            f"crawler:{crawler_id}:progress", "total", progress_data.get("total", 0)
        )
        pipeline.hset(
            f"crawler:{crawler_id}:progress",
            "percentage",
        ...
        pipeline.expire(f"crawler:{crawler_id}:progress", 604800)  # 7 day TTL
        pipeline.execute()
...
def categorize_error(error: Exception) -> CrawlerJobError:
    ...
    if any(
        keyword in error_str
        for keyword in ["memory", "disk", "space", "resource", "quota"]
    ):
    ...
    if any(
        keyword in error_str
        for keyword in ["validation", "invalid", "malformed", "corrupt"]
    ):
...
def run_crawler_job(crawler_id: str, admin_id: str, run_type: str = "manual"):
    ...
    with app.app_context():
        ...
        try:
            ...
            app.logger.info(
                f"✅ Acquired global crawler lock (timeout: {lock_timeout}s)"
            )
            ...
            if crawler.cancel_requested:
                app.logger.info(
                    f"Resetting stale cancel_requested flag for {crawler.name}"
                )
            ...
            async def run_with_heartbeat():
                ...
                async def heartbeat_loop():
                    ...
                    while True:
                        ...
                        if _shutdown_requested:
                            app.logger.warning(
                                "Shutdown requested, stopping crawler..."
                            )
                            if scraper_manager:
            ...
            if result.get("success"):
                ...
                pipeline.hset(
                    f"crawler:{crawler_id}:job", "duration_seconds", int(duration)
                )
                pipeline.hset(
                    f"crawler:{crawler_id}:job",
                    "items_scraped",
            ...
            pipeline.hset(
                f"crawler:{crawler_id}:job", "error_message", categorized_error.message
            )
            pipeline.hset(
                f"crawler:{crawler_id}:job",
                "error_category",
                categorized_error.category,
            )
            pipeline.hset(
                f"crawler:{crawler_id}:job",
                "error_retryable",
                str(categorized_error.retryable),
            )
            pipeline.hset(
                f"crawler:{crawler_id}:job", "failed_at", datetime.utcnow().isoformat()
            )
            pipeline.hset(
                f"crawler:{crawler_id}:job", "duration_seconds", int(duration)
            )
            ...
            pipeline.hset(
                f"crawler:{crawler_id}:error", "traceback", traceback.format_exc()
            )
            pipeline.hset(
                f"crawler:{crawler_id}:error",
                "timestamp",
                datetime.utcnow().isoformat(),
            )
            pipeline.expire(f"crawler:{crawler_id}:error", 604800)  # 7 days
            pipeline.execute()
...
def cancel_crawler_job(crawler_id: str):
    ...
    with app.app_context():
        ...
        job_id = (
            job_id_bytes.decode() if isinstance(job_id_bytes, bytes) else job_id_bytes
        )
        ...
        try:
            ...
            app.logger.info(
                f"✅ Cancelled crawler job {job_id} for crawler {crawler_id}"
            )
