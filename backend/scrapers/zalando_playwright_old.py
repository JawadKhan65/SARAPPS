...
# Increase timeouts and add retry settings (overridden by config if present)
DEFAULT_TIMEOUT = 120000  # 120 seconds
NAVIGATION_TIMEOUT = 120000  # 120 seconds for page navigation
MAX_RETRIES = 4
    ...
    async def scrape(
        ...
        def save_checkpoint():
            try:
                ...
                logger.debug(
                    f"Checkpoint saved (page={state['current_page']}, products={state['products_scraped']})"
                )
            except Exception as e:
        ...
        async with async_playwright() as p:
            ...
            try:
                    ...
                    for local_idx, product_url in enumerate(product_links, 1):
                        ...
                        try:
                            ...
                            if details and details.get("sole_image_url"):
                                ...
                                if state["products_scraped"] % checkpoint_interval == 0:
                                    save_checkpoint()
                            else:
                                state["failed_urls"].append(product_url)

                            global_idx += 1
                            await sleep_product_delay()
                        except Exception as e:
                            logger.error(
                ...
                logger.info(
                    f"✅ Scraped {len(results)} products with sole images (resume-capable)"
                )
            finally:
                try:
                    await context.close()
