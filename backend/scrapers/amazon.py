...
# Make sure models package is importable (workspace layout: backend/models)
...
class AmazonScraper:
    ...
    async def collect_product_links(self, page: Page) -> List[str]:
        ...
        try:
            await page.goto(
                START_URL, wait_until="domcontentloaded", timeout=NAV_TIMEOUT
            )
        ...
        async def extract_links_from_page():
            # Wait for product containers - Amazon uses div[cel_widget_id*="SEARCH_RESULTS"]
            # Look for divs with data-component-type="s-impression-logger" (product containers)
            try:
                ...
                logger.warning(f"Selector timeout waiting for products: {e}")
                # Try to extract what we can without waiting
                pass

            logger.debug("✓ Attempting to load product items")
            ...
            logger.debug(f"Loaded {previous_count} product items after scrolling")

            # Collect product links - each product container has links inside
            items = page.locator('div[data-component-type="s-impression-logger"]')
            ...
            for i in range(count):
                try:
                    ...
                    h2 = item.locator("h2 a.a-link-normal").first
                    href = None
                    ...
                    if await h2.count() > 0:
                        ...
                        anchors = item.locator("a.a-link-normal")
                        for a_idx in range(
                    ...
                    if href not in links:
                        links.append(href)
                        logger.debug(f"Item {i}: Added {href[:80]}")
                except Exception as e:
